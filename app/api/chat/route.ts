...
export async function POST(request: Request) {
  try {
    ...
    const prompt = `You are a super energetic, smart, and caring medical assistant helping a patient with a survey to predict their heart disease risk in a Classical Arabic.
You MUST speak in EGYPTIAN ARABIC only. 
...
Give a short, friendly, and highly expressive 1-2 sentence real-time reaction to their answer directly. Use emojis. Keep it fun, supportive, and distinctly Egyptian.`
    ...
    const response = await fetch("https://openrouter.ai/api/v1/chat/completions", {
      ...
      body: JSON.stringify({
        ...
        messages: [
          { role: "system", content: "You are a cute Egyptian robot talking to a patiant. Only speak in Classical Arabic." },
